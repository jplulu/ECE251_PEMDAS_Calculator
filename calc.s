...
start:
	...
	ldr r1, addr_char_read	// set r1 as the temporary number-char array

read:	
	ldrb r4, [r9], #1	// load the user input byte by byte
	cmp r4, #0x00		// compare r4 with null
	beq convert		// if r4 is null, convert 
...
convert:
	cmp r5, #0		// compare number of num-char read with 0
	beq two_op_check	// if zero, then there are two operators in a row, branch to two_op_check
	...
	beq str0		// branch to the right section to store value read (str0-str4)
	cmp r0, #1
	beq str1
	cmp r0, #2
	beq str2
...
// implement storing floating number read into registers (s10-s14)
str0:
	ldr r2, addr_num_read	// load the address of num-read
...
// compare current operator with the operator on top of the stack, perform op on top of stack if it has same or greater precidence
mulcheck:
	ldr r0, addr_op_count	// load the addr of op-counter
	ldr r0, [r0]		// deref
	...
	beq divide
	b pushop		// branch to push op onto op_stack
...
right:
	ldr r0, addr_op_stack	// load the addr of op-stack 
	ldr r1, addr_op_count	// load the addr of op-counter
	ldr r1, [r1]
...
// perform all operations on the op_stack until no operations are left
stackcheck:
	ldr r0, addr_op_count	// load the addr of op-count
	ldr r0, [r0]		
	cmp r0, #0		// compare with 0
...
// check how many elements in the val-array and branch to perform operation on correct registers
multiply:
...
popleftpar:
	ldr r0, addr_op_count	// load the addr of op_count
	ldr r1, [r0]		// deref
	sub r1, r1, #1		// decrement the counter by 1
	str r1, [r0]		// store back to addr of op_count
	b start
...
getresult:
	fcvtds d0, s10		// convert the float to double precision
	vmov r2, r3, d0		// store the double into two registers
	ldr r0, addr_msg	// load the address of the output
